...
void injectImportFile(FILE *inputFile, FILE *outputFile, int *processedFiles, const char *codePath,
    ...
    FILE *importFile = fopen(importPath, "r");
    ...
    if(importFile == NULL){
        ...
        preprocR(importFile, outputFile, processedFiles, importCodePath, importPath);
...
void outputImport(FILE **inputFile, FILE **outputFile, char *currChar, int *injectionWhitespace,
                    ...
                    int *inDoubleQuote, int *importContextStart, int *processedFiles){

                    // Print a white space character for each character you deal with to keep the
                    // column value intact
                    ...
                    if(*currChar == '\n' || *currChar == '\r'){
                        ...
                        // Temp
                        preprocError(*inputFile, *outputFile, STR_ERROR_000002, codePath, filePath,
                                        line, *column, 1, 1);

                    }else if(inQuote){ // Check if you're in a quoting context

                        // Temp
                        preprocError(*inputFile, *outputFile, STR_ERROR_000003, codePath, filePath,
                                        line, *column, 1, 0);
                        ...
                        *currChar = fgetc(*inputFile);
                        ...
                        while(*currChar != '"'){
                            ...
                            if(*currChar == '\r' || *currChar == '\n'){
                                ...
                                preprocError(*inputFile, *outputFile, STR_ERROR_000007, codePath,
                                                filePath, line, (*column) - 1, 1, 1);
                        ...
                        *currChar = fgetc(*inputFile);
                        ...
                        if(*currChar != ';'){
                            ...
                            injectImportFile(*inputFile, *outputFile, processedFiles, codePath,
                                                filePath, line, startColumn, importPath,
                                                absoluteImportPath);

                            // Free up the memory used by the `absoluteImportPath` and `importPath`
                            // variables
                        ...
                        // Temp
                        preprocError(*inputFile, *outputFile, STR_ERROR_000004, codePath, filePath,
                                        line, *column, 1, 0);
